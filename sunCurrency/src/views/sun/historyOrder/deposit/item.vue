<template>
  <div class="PoolItem">
    ...
    <div class="PoolItem-middle">
      <template v-for="(item, key) in displayData" :key="key">
        <div class="flex-row PoolItem-middle-tb" v-if="item.isShow">
    ...
    <div class="PoolItem-bottom" v-if="item.type === 1">
      <SunButton class="onbtn" type="primary" :loading="loading" shape="round" size="large" @click="rewithdrawPledge"> {{ $t('index.redemption_4') }} </SunButton>
    </div>
  </div>
</template>
...
const displayData = computed(() => {
  const displayArray = [
    ...
    {
      key: 'trial_amount',
      value: props.item.trial_amount + ' ' + props.item.currency.name,
      isShow: props.item.trial_amount > 0,
    },
    {
      key: 'daily_rate',
