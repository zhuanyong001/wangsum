<template>
  <sun-modal :title="$t('app.popup_deposit_title')" @ok="onSubmit" @cancel="handleCancel" :show-footer="false">
    <div class="sunModal">
      <form @submit.prevent="onSubmit">
        <div class="form-group">
          <label for="username">{{ $t('app.popup_select_coin') }}</label>
          <sun-select ref="sunSelectRefLoan" cancelled :items="Coins" @change="currencyTypeChange" columns="repeat(4, 1fr)"></sun-select>
        </div>
        <!-- <div class="form-group">
          <label for="username">{{ '选择存款方式' }}</label>
        ...
        <div class="form-group" v-if="params.cycle !== CurrentId && Times.length > 0">
          <label for="username">{{ $t('app.popup_select_time') }}</label>
          <sun-select ref="sunSelectRefPledge" cancelled :items="Times" @change="TimeChange" columns="repeat(4, 1fr)"></sun-select>
        </div>
        <div class="form-group">
          <div class="flex-row">
            <label for="username">{{ $t('app.popup_input_deposit') }}</label>
          ...
          <div class="sun-a-number-input sun-a-number-input-square">
            <a-input-number  v-model:value="params.amount" :bordered="false" size="large" :placeholder="$t('index.mount_4')" :controls="false">
              <template #addonAfter>
        ...
        <div class="sunModal-middle">
          <div class="flex-row sunModal-middle-tb">
            <div class="title">{{ $t('app.popup_deposit_ratio') }}</div>
            <div class="num">{{ APY }}</div>
          </div>
          <div class="flex-row sunModal-middle-tb">
            <div class="title">{{ $t('loan.day_income') }}</div>
            <div class="num">{{ baseDayRate }}</div>
          </div>
          <div class="flex-row sunModal-middle-tb" v-if="extraRate">
            <div class="title">{{ $t('loan.day_income') }}</div>
            <div class="num">{{ extraRate }}</div>
          </div>
        </div>
        <div class="btn">
          <SunButton class="onbtn" type="primary" @click="onSubmit" shape="round" size="large" block> {{ $t('index.confirm_11') }} </SunButton>
        </div>
      </form>
    </div>
  </sun-modal>
...
const params = ref<pledgePost>({
  ...
  currency_id: props.attrs.coins[0].id || 0, //币种id
  cycle: props.attrs.cycles[0].id || 0, //周期id
  amount: 0, //数量
})
...
const { balance, APY, baseDayRate, extraRate } = FinancialCalculations(params, props.attrs, Times)
