...
export const useMember = ({ getData }) => {
  ...
  const setLevel = (form = {}) => {
    let level = [{ label: '默认', value: 0 }, ...LevelList.value];
    //console.log('====>>level', level);

    const formConfig = reactive([{ label: '会员等级', key: 'level', type: 'select', desc: '', opts: level }]);
    const { id } = form;
    modal
      .open({
        props: {
          formConfig,
          title,
          submit: (formData) => submit(formData, '/web3/members/set/membership_level'),
        },
      })
      .Init({ user_id: id });
  };

  const setRemark = (form = {}) => {
    const { id, remark } = form;
    const formConfig = reactive([{ label: '会员备注', key: 'remark', type: 'textarea', desc: '' }]);
    modal
      .open({
        props: {
          formConfig,
          title,
          submit: (formData) => submit(formData, '/web3/members/set/remark'),
        },
      })
