<template>
  <div>
    <a-modal :title="'推送客户'" v-model:visible="showModal" @ok="submit" :width="600" :confirmLoading="loading">
      <a-form ref="formModel" :model="formData" :labelCol="{ span: 5 }" :wrapperCol="{ span: 16 }">
        <a-form-item label="接待号" name="whatsapp">
          {{ curr_item.whatsapp }}
        </a-form-item>
        <a-form-item label="whatsapp" name="whatsapp" required>
          <a-input v-model:value="formData.whatsapp" />
        </a-form-item>
...
<script setup>
  ...
  const submit = async () => {
    let url = '/oa/df_agent/add_fans';
    const form = await formModel.value?.validate();
    const params = {
      ra_id: curr_item.value.id,
      ...formData,
    };
    loading.value = true;
    try {
