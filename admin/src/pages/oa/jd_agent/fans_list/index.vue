  ...
  >
    ...
    <template #bodyCell="{ text, record, column }">
      ...
      <template v-else-if="column.dataIndex === 'action'">
        ...
        <a-button
          v-if="record.kaihu_status == 0"
          class="text-xs ml-base"
          type="primary"
          size="small"
          @click="setStatus(record, 1)"
          :loading="record.loading"
          >申请开户</a-button
        >
        <a-popconfirm title="确认？" @confirm="setStatus(record, -1)" v-if="record.kaihu_status == 0">
          <a-button class="text-xs ml-base" type="" size="small" danger :loading="record.loading">标记失败</a-button>
        </a-popconfirm>
